...
from learning_to_be_taught.environments.meta_world.ml10_v2 import ML10V2
from learning_to_be_taught.environments.meta_world.ml45_v2 import ML45V2
from learning_to_be_taught.environments.meta_world.ml1_v2 import ML1V2
from learning_to_be_taught.environments.meta_world.ml3_v2 import ML3V2


class DemonstrationsMetaWorld(gym.Env):
    def __init__(self, demonstrations_flag=True, benchmark='ml10', action_repeat=2, time_observation=True, **kwargs):
        assert benchmark in ['ml1', 'ml3', 'ml10', 'ml45'], 'benchmark not available'
        if benchmark == 'ml1':
            self.benchmark = ML1V2(**kwargs)
        elif benchmark == 'ml3':
            self.benchmark = ML3V2(**kwargs)
        elif benchmark == 'ml10':
            self.benchmark = ML10V2(**kwargs)
        elif benchmark == 'ml45':
            self.benchmark = ML45V2(**kwargs)
            ...
            self.observation_space = gym.spaces.Dict({
                'state': gym.spaces.Box(low=0, high=1, shape=(self.state_length,)),
            ...
            self.observation_space = gym.spaces.Box(low=0, high=1, shape=(self.state_length,))
...
if __name__ == '__main__':
    s = time.time()
    env = MetaWorld(benchmark='ml45', demonstrations_flag=True, time_observation=True, sample_num_classes=1)
